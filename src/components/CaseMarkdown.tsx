...
function renderBlock(block: string, index: number) {
  ...
  if (embedMatch) {
    const src = caseMediaSrc(embedMatch[1]);
    return (
...
function parseBodyParts(body: string): ArticleBodyPart[] {
  ...
  for (const block of blocks) {
    ...
    if (imgMatch) {
      flushText();
      gallery.push(caseMediaSrc(imgMatch[1]));
      continue;
    }
...
export default function CaseMarkdown({ content }: CaseMarkdownProps) {
